;**************************************************************
; FINNv2 Emissions Calculation - Version 2.5.1
...
; 4 April 2023: for fires processed including with and 
;               without large-polygon fires
; 3 Nov 2023: Setting up to run on acom-finn in real-time
;    FRP now available from the fire preprocessing, 
;        saving it to a separate file.
;**************************************************************

pro finn2_calc_emissions_v251

  args = COMMAND_LINE_ARGS()

  if n_elements(args) ne 3 then begin
    print, 'Usage: idl -e finn2_calc_emissions_v251 -args <date> <raster_year> <data_folder>'
  ...
  path_in = args[2]
  file_in = path_in + '/out_'+simid+'_'+date_lab+'_modlct_'+syrlc+'_modvcf_'+syrlc+'_regnum_frp.csv'
  ...
  ;log_genveg = path_out + 'LOG_genveg_calcemis_FINN'+finnver+'_'+simid+'_'+sdate_emis+'_'+todaystr+'.txt'
  ...
  file_fuelloads = path_in + '/emis_inputs/Fuel_LOADS_NEW_022019.csv' 
  file_usfuel = path_in + '/emis_inputs/LCTFuelLoad_fuel4_revisit20190521.csv'
  file_efs = path_in + '/emis_inputs/EFs_byGenVeg_c20210601.csv'
  file_VOCsplit_M = path_in + '/emis_inputs/NMOCfrac_byGenVeg_MOZ.csv'
  file_VOCsplit_S = path_in + '/emis_inputs/NMOCfrac_byGenVeg_SAPRC.csv'
  file_VOCsplit_G = path_in + '/emis_inputs/NMOCfrac_byGenVeg_GEOSCHEM.csv'
  ...
  ;v2.5.1 file:
  ;polyid fireid cen_lon cen_lat acq_date_utc area_sqkm alg_agg v_lct f_lct v_tree v_herb v_bare v_regnum
  ...
  for i=0L,nfires-1 do begin
   ...
   if (n_elements(parts) lt 12) then begin
   ...
   herb = Float(parts[10])
   bare = Float(parts[11])
   if (n_elements(parts) eq 14) then begin
